...
class AggregateAttention(nn.Module):
    def __init__(self, in_ch, num_classes, head_num=1, dropout=0.2):
        super(AggregateAttention, self).__init__()
        self.attention = MultiHeadAttention(in_ch, head_num, dropout, version='v2')
        self.register_buffer('t_bank', torch.randn(num_classes, 512))
        self.seed_project_left = V2G_linear(in_ch)
        for i in self.seed_project_left:
                if isinstance(i, nn.Linear):
                    nn.init.normal_(i.weight, std=0.01)
                    nn.init.constant_(i.bias, 0)
        self.num_classes = num_classes

    def aggregate(self, nodes, labels):
        uni_labels = labels.unique(sorted=True).long()
        aggregated_nodes = torch.zeros(self.num_classes, nodes.size(1)).to(nodes.device)
        ...
        for label in uni_labels:
            mask = (labels == label).unsqueeze(-1)  
            selected_nodes = nodes * mask.float()
            bs = selected_nodes.sum(dim=0) / mask.sum()
            momentum = F.cosine_similarity(bs.unsqueeze(0), self.t_bank[label].unsqueeze(0))

            with torch.no_grad():
                new_value = self.t_bank[label] * momentum + bs * (1.0 - momentum)
                self.t_bank[label] = new_value

            # self.t_bank[label] = self.t_bank[label] * momentum + bs * (1.0 - momentum)
            aggregated_nodes[label] = bs

        for label in missing_labels:
            aggregated_nodes[label] = self.t_bank[label]

        return aggregated_nodes
    
    def forward(self, feats, labels, attn_mask=None):
        # feats = feats.clone().detach()
        nodes = self.seed_project_left(feats)
        nodes, _ = self.attention([nodes, nodes, nodes], attn_mask)
        nodes_cls = self.aggregate(feats, labels)
        
        return nodes, nodes_cls
