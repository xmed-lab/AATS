...
# from .GCN import GCN
# from .loss import GraphConLoss, SupConLoss
...
class DAobjTwoStagePseudoLabGeneralizedRCNN(GeneralizedRCNN):
    ...
    def __init__(
        ...
        *,
        ...
        # add by Lvxg
        # self.GraphGCN = GCN(1024, 1024, 1024)
    ...
    def forward(
        self, batched_inputs, branch="supervised", given_proposals=None, val_mode=False, rpn_t=None, roi_t=None, fea_t=None
    ):
        """
        Args:
        ...
        if branch.startswith("supervised"):
        ...
        elif branch.startswith("unsup_target_strong"):
            ...
            if fea_t is not None and roi_t is not None:
                #############################mod 1 
                # box_feature_t = self.roi_heads._shared_roi_transform([fea_t['vgg4']], [x.proposal_boxes for x in rpn_t]) # (4000, 512, 7, 7)
                # logits_roi_t = self.roi_heads.box_predictor(box_feature_t)
                # graph_feat_t = self.GraphGCN(box_feature_t) # (4000, 1024)
                # logits_graph_t = self.roi_heads.box_predictor(graph_feat_t) # (4000, 10) * 2

                # box_feature_s = self.roi_heads._shared_roi_transform([features['vgg4']], [x.proposal_boxes for x in proposals_rpn])
                # logits_roi_s = self.roi_heads.box_predictor(box_feature_s)
                # graph_feat_s = self.GraphGCN(box_feature_s)
                # logits_graph_s = self.roi_heads.box_predictor(graph_feat_s)

                # losses["loss_con_st"] = self.KD_loss(logits_roi_t, logits_roi_s)
                # losses["loss_con_s_graph"] = self.KD_loss(logits_graph_s, logits_roi_s) * 0.1
                # losses["loss_con_t_graph"] = self.KD_loss(logits_graph_t, logits_roi_t) * 0.1
                # losses["loss_graph"] = self.Graph_conloss(box_feature_t, box_feature_s, self.GraphGCN) * 0.1

                ##############################mod2
                box_feature_s = self.roi_heads._shared_roi_transform([features['vgg4']], [x.proposal_boxes for x in proposals_rpn])
                logits_roi_s = self.roi_heads.box_predictor(box_feature_s, branch) # 
                roi_s = self.roi_heads.box_predictor.inference(logits_roi_s, proposals_rpn)
            return losses, roi_s[0], features
    ...
    def KD_loss(self, logits1, logits2):
        ...
        if isinstance(logits2, tuple):
            logits2 = logits2[0]
        prob1 = F.log_softmax(logits1, dim=1) # teacher
        prob2 = F.softmax(logits2, dim=1) # student
        kd_loss = F.kl_div(prob1, prob2, reduction='batchmean') # prob2.detach
        return kd_loss
